...
resetarSenha.post("/resetar-senha/:tokenSenha", async (request, response) => {
  ...
  try {
    ...
    if (senha === "" && confirmaSenha === "") {
      ...
      const teste = await tabelaUsuario.update(
        {
          senha: hashSenha, //Altera o campo senha no banco com o hashSenha
        },
        { where: { tokenSenha: request.params.tokenSenha } }
      ); // Realiza a alteração somente quando o tokenSenha no banco for igual ao tokenSenha da requisição
